...
function App() {
  ...
  return (
    <>
      <PresentationCard />
      <div className='mt-10'>
        {experiences.map((experience, idx) =>  <JobExperienceSection key={`job-${idx}`} {...experience}/>)}
      </div>     
      <Swiper
        slidesPerView={1}
        ...
        spaceBetween={50}
      >
        {projects.map((project, idx) => <SwiperSlide key={`project-slide-${idx}`}><ProjectSection {...project}/></SwiperSlide>)}
      </Swiper>
    </>
  )
}
